...
rm -rf ./server/*
rm -rf ./client/*
cp fabric-ca-server-config2.yaml ./server/fabric-ca-server-config.yaml
...
ATTRIBUTES='"hf.Registrar.Roles=peer,user,client","hf.AffiliationMgr=true","hf.Revoker=true","hf.Registrar.Attributes=*"'
fabric-ca-client register --id.type client --id.name test-admin --id.secret adminpw --id.affiliation test --id.attrs $ATTRIBUTES
...
export FABRIC_CA_CLIENT_HOME=$PWD/client/test/admin
fabric-ca-client enroll -u http://test-admin:adminpw@localhost:7054
mkdir -p $FABRIC_CA_CLIENT_HOME/msp/admincerts
cp $FABRIC_CA_CLIENT_HOME/../../caserver/admin/msp/signcerts/*  $FABRIC_CA_CLIENT_HOME/msp/admincerts
...
mkdir -p ./client/test/msp/admincerts
mkdir ./client/test/msp/cacerts
mkdir ./client/test/msp/keystore
cp $ROOT_CA_CERTIFICATE ./client/test/msp/cacerts
cp ./client/test/admin/msp/signcerts/* ./client/test/msp/admincerts  
...
export FABRIC_CA_CLIENT_HOME=$PWD/client/test/admin
fabric-ca-client register --id.type peer --id.name test-peer1 --id.secret adminpw --id.affiliation test
export FABRIC_CA_CLIENT_HOME=$PWD/client/test/peer1
fabric-ca-client enroll -u http://test-peer1:adminpw@localhost:7054
cp -a $PWD/client/test/admin/msp/signcerts  $FABRIC_CA_CLIENT_HOME/msp/admincerts
...
export FABRIC_CA_CLIENT_HOME=$PWD/client/acme/admin
ATTRIBUTES='"hf.AffiliationMgr=false:ecert","hf.Revoker=false:ecert","department=logistics:ecert","app.logistics.role=specialis:ecert"'
fabric-ca-client register --id.type user --id.name gandalf --id.secret pw --id.affiliation acme --id.attrs $ATTRIBUTES
export FABRIC_CA_CLIENT_HOME=$PWD/client/acme/gandalf
fabric-ca-client enroll -u http://gandalf:pw@localhost:7054
cp -a $PWD/client/acme/admin/msp/signcerts  $FABRIC_CA_CLIENT_HOME/msp/admincerts
...
ANCHOR_UPDATE_TX=./config/airline-anchor-update-test.tx
configtxgen -profile AirlineChannel -outputAnchorPeersUpdate $ANCHOR_UPDATE_TX -channelID airlinechannel -asOrg TestMSP
